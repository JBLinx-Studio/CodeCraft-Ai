            ...
            <div className="container py-16 px-4 text-center relative z-10">
              <div className="max-w-3xl mx-auto">
                ...
                <div className="flex flex-wrap justify-center gap-3 mb-8 animate-fade-in">
                  {["React", "Tailwind CSS", "TypeScript", "Modern UI"].map((tech) => (
              ...
              <div className="border-b bg-white/50 dark:bg-black/20">
                <TabsList className="w-full justify-start bg-transparent">
                  <TabsTrigger 
                    value="chat" 
                    className={cn(
                      "rounded-t-lg border-b-2 border-transparent py-3 px-4",
                      "data-[state=active]:border-primary data-[state=active]:bg-transparent",
                      "data-[state=active]:shadow-none font-medium"
                    )}
                  >
    ...
    <div className="container my-12">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        <FeatureCard
          ...
          description="Production-ready code that follows industry best practices and patterns"
        />
        
        <FeatureCard
          icon={<Layout className="h-5 w-5 text-primary" />}
          title="Real-time Preview"
          description="Instantly see your application with professional-grade UI components"
        />
        
        <FeatureCard
          icon={<Zap className="h-5 w-5 text-primary" />}
          title="Production Export"
          description="Deploy-ready code with optimized performance and accessibility"
        />
      </div>
...
const FeatureCard = ({ icon, title, description }: { icon: React.ReactNode; title: string; description: string }) => {
  return (
    <div className="premium-card p-6 transition-all duration-300 hover:shadow-hover">
      ...
      <h3 className="font-heading font-semibold mb-2 text-lg">{title}</h3>
  ...
  const steps = [
    {
      ...
      description: "Specify your application needs with detailed requirements"
    },
    {
      icon: <Zap className="h-5 w-5 text-white" />,
      title: "2. Generate Solution",
      description: "Our AI creates enterprise-grade code that meets your specifications"
    },
    {
      icon: <Layout className="h-5 w-5 text-white" />,
      title: "3. Deploy & Scale",
      description: "Export production-ready code that's optimized for performance"
    }
  ];
    ...
    <div className="mt-24 mb-12">
      ...
      <div className="flex flex-col md:flex-row justify-between max-w-4xl mx-auto relative">
        ...
        {steps.map((step, index) => (
          <div key={index} className="flex-1 flex flex-col items-center text-center p-6 relative z-10 animate-fade-in">
