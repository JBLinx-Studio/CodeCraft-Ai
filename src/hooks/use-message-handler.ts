...
type GenerateCodeFunction = (content: string) => Promise<any>;
type SetAuthErrorFunction = React.Dispatch<React.SetStateAction<boolean>>;
type CodeGeneratedCallback = (html: string, css: string, js: string) => void;

export const useMessageHandler = (
  setMessages: SetMessagesFunction, 
  generateCode: GenerateCodeFunction,
  setHasAuthError: SetAuthErrorFunction,
  onCodeGenerated: CodeGeneratedCallback
) => {
  ...
  const handleMessageGeneration = async (content: string) => {
  ...
  return { addMessage, handleMessageGeneration };
};
